...
if(Yii::app()->user->getState("employee_email") == 'jayesh.menon@kritva.com' )
{
...
//print_r(count($chk_user_cls1));die();
}
    ...
    //$query  = Yii::app()->db->createCommand("SELECT * FROM Employee where retire_date< '2017-03-31' and Employee_status != 'Resign' and Employee_id NOT IN ('.$serial_nos.') order by 'Cadre'" );
//$chk_user_cls11 = $query->queryAll();
...
$where = 'where bu_head_email = :bu_head_email and Employee_status != :Employee_status  and pms_status !=:pms_status order by `cluster_name`';
					...
					{
					    if(date('Y-m-d',strtotime($chk_user_cls21[$e]['joining_date']))<'2016-10-01' && $chk_user_cls21[$e]['joining_date']!="#N/A")
            			{
					...
					{
					    if(date('Y-m-d',strtotime($chk_user_cls31[$e]['joining_date']))<'2016-10-01' && $chk_user_cls31[$e]['joining_date']!="#N/A")
            			{
...
$chk_user_cls = $chk_user_cls3;
               ...
               //print_r($chk_user_cls);die();         
            if(count($employee_result)>0)
			{				
				for ($j=0; $j < count($chk_user_cls); $j++) 
				{ 
					if (isset($chk_user_cls[$j]['Employee_id'])) {
						$where1 = 'where Employee_id = :Employee_id ORDER BY `rating` DESC';
						$list1 = array('Employee_id');
						$data2 = array($chk_user_cls[$j]['Employee_id']);
