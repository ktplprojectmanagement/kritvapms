...
class LoginForm extends CActiveRecord
{
	...
	public function check_login($data)
	{
		...
		$sql = "select * from login where (username = :username or Employee_id = :username) and password = :password";
...
$command->bindValue(":Employee_id",$data['username'],PDO::PARAM_STR);
		...
		//return $find_result;
		if ($find_result != '') {
			$sql1 = "select * from Employee where Email_id = :Email_id or Employee_id = :Employee_id";
...
public function demo_role($data)
	{
		//print_r($data);die();
		$connection=Yii::app()->db;
		$sql = "select * from login where (username = :username or Employee_id = :username)";
		$command=$connection->createCommand($sql);
		$command->bindValue(":username",$data['username'],PDO::PARAM_STR);
$command->bindValue(":Employee_id",$data['username'],PDO::PARAM_STR);
		//$result = $command->execute();
	...
	public function check_admin($data)
	{
		//print_r($data);die();
		$connection=Yii::app()->db;
		$sql = "select * from login where username = :username and password = :password and role_id = :role_id";
		$command=$connection->createCommand($sql);
		$command->bindValue(":username",$data['username'],PDO::PARAM_STR);
		$command->bindValue(":password",md5($data['password']),PDO::PARAM_STR);
		$command->bindValue(":role_id",'4',PDO::PARAM_STR);
		//$result = $command->execute();
		$find_result = $command->queryRow();
		return $find_result;
	}

	public function check_role1($data)
	{
		//print_r($data);die();
		$connection=Yii::app()->db;
		$sql = "select * from login where (username = :username or Employee_id = :username) and password = :password and first_login_flag = :first_login_flag";
		$command=$connection->createCommand($sql);
		$command->bindValue(":username",$data['username'],PDO::PARAM_STR);
                $command->bindValue(":Employee_id",$data['username'],PDO::PARAM_STR);
		$command->bindValue(":password",md5($data['password']),PDO::PARAM_STR);
		$command->bindValue(":first_login_flag",1,PDO::PARAM_STR);
		//$result = $command->execute();
		$find_result = $command->queryRow();
		return $find_result;
	}

	function get_login_data($where,$data,$list)
	{		
		$connection=Yii::app()->db;
